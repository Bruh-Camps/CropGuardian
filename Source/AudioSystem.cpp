...
AudioSystem::AudioSystem(int numChannels)
{
	...
	{
		SDL_Log("[AudioSystem] Mix_OpenAudio falhou: %s", Mix_GetError());
	}
...
void AudioSystem::Update(float deltaTime)
{
	...
	{
		// Se o canal está marcado como válido (ou seja, estava tocando algo)
		if (mChannels[i].IsValid())
		{
			// Verifica se o canal ainda está tocando
			if (Mix_Playing(static_cast<int>(i)) == 0)
			{
				// Canal finalizou a execução do som
				SoundHandle handle = mChannels[i];

				// Remove do mapa de sons ativos, se existir
...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping)
{
	...
	if (!sound)
	{
		SDL_Log("[AudioSystem] Error: Sound %s could not be loaded", soundName.c_str());
	...
	if (availableChannel == -1)
	{
		for (auto it = mHandleMap.begin(); it != mHandleMap.end(); ++it)
		{
			if (it->second.mSoundName == soundName)
			{
				...
				SDL_Log("[AudioSystem] Reusing channel %d from sound %s", availableChannel, soundName.c_str());
				StopSound(it->first);
				break;
			}
		}
	...
	if (availableChannel == -1)
	{
		for (auto it = mHandleMap.begin(); it != mHandleMap.end(); ++it)
		{
			...
			{
				availableChannel = it->second.mChannel;
				SDL_Log("[AudioSystem] Stopping looping sound %s on channel %d", it->second.mSoundName.c_str(), availableChannel);
				StopSound(it->first);
				break;
			}
		}
	...
	if (availableChannel == -1 && !mHandleMap.empty())
	{
		...
		availableChannel = it->second.mChannel;
		SDL_Log("[AudioSystem] Stopping oldest sound %s on channel %d", it->second.mSoundName.c_str(), availableChannel);
		StopSound(it->first);
	}
	...
	{
		SDL_Log("[AudioSystem] Failed to play sound %s on channel %d", soundName.c_str(), availableChannel);
...
void AudioSystem::StopSound(SoundHandle sound)
{
	...
	if (it == mHandleMap.end())
	{
		SDL_Log("[AudioSystem] StopSound: Sound handle %s not found", sound.GetDebugStr());
		return;
	}
	int channel = it->second.mChannel;

	Mix_HaltChannel(channel); // Para o som
	mHandleMap.erase(it); // Remove o handle do mapa
	mChannels[channel].Reset(); // Reinicializa o canal
...
void AudioSystem::PauseSound(SoundHandle sound)
{
	...
	if (it == mHandleMap.end())
	{
		SDL_Log("[AudioSystem] PauseSound: Sound handle %s not found", sound.GetDebugStr());
		return;
	}
...
void AudioSystem::ResumeSound(SoundHandle sound)
{
	...
	if (it == mHandleMap.end())
	{
		SDL_Log("[AudioSystem] ResumeSound: Sound handle %s not found", sound.GetDebugStr());
		return;
	}
	HandleInfo& info = it->second;
