...
bool UIFont::Load(const std::string& fileName)
{
    ...
    for(int fontSize: fontSizes) {
      TTF_Font* font = TTF_OpenFont(fileName.c_str(), fontSize);
    	if (font == nullptr) {
    		SDL_Log("Failed to load font size %d: %s", fontSize, TTF_GetError());
    		return false;
    	}
    	mFontData[fontSize] = font;
     }
	return true;
}
                                ...
                                int pointSize /*= 24*/, unsigned wrapLength /*= 900*/)
{
    ...
    if(mFontData.find(pointSize) == mFontData.end()) {
        SDL_Log("Font size not supported: %d", pointSize);
        return nullptr;
      }
    surface = TTF_RenderUTF8_Blended_Wrapped(mFontData[pointSize], text.c_str(), sdlColor, wrapLength);
	...
	SDL_FreeSurface(surface); // Libera memória da superfície
      if (texture == NULL) {
